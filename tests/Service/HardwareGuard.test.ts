...
describe('HardwareGuard', () => {
    ...
    describe('constructor', () => {
        it('succeeds with logger and mins', () => {
            expect(() => new HardwareGuard(AppLogger.getInstance(), 3.8, '12.0.0')).not.toThrow();
        });

        it('throws HardwareGuardException when minVersion is not parseable', () => {
            expect(() => new HardwareGuard(AppLogger.getInstance(), 3.8, '')).toThrow(HardwareGuardException);
            expect(() => new HardwareGuard(AppLogger.getInstance(), 3.8, 'invalid')).toThrow(/minSemver is not parseable/);
        });
    });
    ...
    describe('isCompatible', () => {
        it('returns true on iOS when version, RAM and 64-bit are satisfied (iOS matrix)', () => {
            guard = new HardwareGuard(AppLogger.getInstance(), 3.8, '12.0.0');
            setPlatform('ios');
            mockDevice.osVersion = '15.0';
            mockDevice.totalMemory = 4 * 1024 ** 3;
            mockDevice.supportedCpuArchitectures = ['arm64'];
        ...
        it('returns true on Android when version (>=9.0.0), totalMemory (>=6GB) and 64-bit are satisfied (Android matrix)', () => {
            guard = new HardwareGuard(AppLogger.getInstance(), 6, '9.0.0');
            setPlatform('android');
        ...
        it('returns true when custom mins are passed (lower bar)', () => {
            guard = new HardwareGuard(AppLogger.getInstance(), 2, '11.0.0');
            setPlatform('ios');
            mockDevice.osVersion = '11.0';
        ...
        it('returns false on iOS when version below passed minimum', () => {
            guard = new HardwareGuard(AppLogger.getInstance(), 6, '14.0.0');
            setPlatform('ios');
...
describe('DeviceCompatibilityGate', () => {
    ...
    it('succeeds with default matrix', () => {
        expect(() => new DeviceCompatibilityGate(AppLogger.getInstance())).not.toThrow();
    });
});
