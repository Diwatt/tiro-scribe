...
### Agent Behavior (Karpathy Guidelines)
- **Think Before Coding:** Never guess. If requirements or context are ambiguous, STOP and ask the user for clarification. Do not make silent assumptions.
- **Surgical Changes:** Modify ONLY the lines of code strictly necessary to complete the request. Leave adjacent code exactly as it is.
- **Simplicity First:** Write the most straightforward and minimal code possible to solve the problem. Avoid premature optimization, over-engineering, or creating unnecessary abstractions.
- **Self-Correction:** If your code fails or causes an error, analyze the logs deeply before attempting a fix. Do not blindly guess or iterate rapidly without understanding the root cause.
...
- **Sequential Thinking:** Use this tool to break down complex architectural changes, map out logic, or plan MVVM refactoring before writing any code.
...
### AI & Formatting Behaviors
- **NO unsolicited refactoring:** DO NOT clean up, reformat, or "improve" unrelated code surrounding the lines you are modifying.
- **NO terminal formatting:** DO NOT run `pnpm run biome` or `pnpm run lint`. The user handles formatting locally via `Shift+Alt+S`.
- **NO ignore Biome/ESLint errors:** Fix styling, import, or typing issues in your generated code before finalizing.
