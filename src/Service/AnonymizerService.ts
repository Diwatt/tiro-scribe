...
export class AnonymizerService {
  ...
  private async detectEntitiesWithAI(
    ...
    if (!this.onnxRuntime) {
      console.warn('ONNX Runtime not initialized. Skipping AI-based NER.');
      return [];
    }
  ...
  private detectAndFuzzTemporal(
    ...
    if (!this.sessionStartDate) {
      console.warn('Session start date not set. Temporal fuzzing disabled.');
      return entities;
    }
  ...
  private convertToRelativeDate(dateString: string): string | null {
    ...
    try {
      ...
      const daysDiff = differenceInDays(parsedDate, this.sessionStartDate);
