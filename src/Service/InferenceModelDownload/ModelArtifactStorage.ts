...
export class ModelArtifactStorage {
    ...
    private extractFilenameFromUrl(url: string): string {
        try {
            ...
            if (error instanceof Error) {
                throw new InferenceModelDownloaderException(`Failed to extract filename from URL "${url}": ${error.message}`, error);
            }
    ...
    public hasAllFiles(config: ModelConfig): boolean {
        if (config.files.length === 0) {
            throw new InferenceModelDownloaderException(`Model configuration ${config.capability} (${config.id}) has no files`);
        }
